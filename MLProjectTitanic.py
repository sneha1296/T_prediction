...
Contents:                                             # steps to do for this project

1)Import Necessary Libraries
2)Read In and Explore the Historic Data               # pandas to be used
3)Data Analysis
4)Data Visualization                                  # advanced library of matpoltlib seaborn
5)Cleaning Data                                       # feature testing
6)Choosing the Best Model                             # choose from the spot check algorithm
7)Creating Submission File                            #  create the answer file after testing the algorithm
...
print train.describe() # include text too as by default it takes numeric
...
print
print train.sample(10) # to see the data if data not available
...
# We'll probably drop these values from our data sets.
...
print
print pd.isnull(train).sum()  # gives the column wise summation of null values in count in the form of matrix of 0 and 1
...
#Name             0
#Sex              0
#Age            177   # work to fill this column
#SibSp            0
#Parch            0
#Ticket           0
#Fare             0
#Cabin          687   # remove this column as not needed
#Embarked         2   # fill with southampton as max passesngers are there

#We can see that except for the above mentioned missing values, no NaN values exist.


#Relationship between Features and Survival  # input nd output
...
sbn.barplot(x="Sex", y="Survived", data=train)      # plots the graphs using matplotlib only data and axes are needed
plt.show()
...
train["Age"] = train["Age"].fillna(-0.5)  # fill no available values with -0.5
test["Age"] = test["Age"].fillna(-0.5)
bins =   [-1, 0, 5, 12, 18, 24, 35, 60, np.inf] # numpy.infinite
labels = ['Unknown', 'Baby', 'Child', 'Teenager', 'Student', 'Young Adult', 'Adult', 'Senior']
# -1 to 0 = unknown , 0 to 5 = baby..
train['AgeGroup'] = pd.cut(train["Age"], bins, labels = labels) # cut func of pandas to categorize the age
# we have added new column agegroup which has category on tha basis of age, in both train and test data
...
train["CabinBool"] = (train["Cabin"].notnull().astype('int'))     # if value not null then converted to integer
...
#3) Around 20.5% of the Age feature is missing, we will need to fill that in.
...
print pd.crosstab(train['Title'], train['Sex'])           # crosstab bifurcates the column into subcplomns based on given input
...
print "\n Map each of the title groups to a numerical value. " # as texts arE not allowed in machine learning
title_mapping = {"Mr": 1, "Miss": 2, "Mrs": 3, "Master": 4, "Royal": 5, "Rare": 6}
for dataset in combine:
    dataset['Title'] = dataset['Title'].map(title_mapping) # searches then if matched then replace called mapping thru map function
...
train['FareBand'] = pd.qcut(train['Fare'], 4, labels = [1, 2, 3, 4]) # qcut divides every value of fare column into 4 segments
